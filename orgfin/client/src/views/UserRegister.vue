id="input"<template>
    <div class="container">
      ...
      <div id="area-message">
        <div v-if="messageSuccess" id="success" class="success">{{ messageSuccess }}</div>
        <div v-if="messageError" id="error" class="error">{{ messageError }}</div>
      </div>
  ...
  export default {
    ...
    methods: {
      register() {
        ...
        axios
          .post('http://127.0.0.1:4000/user/register', data)
          .then(res => {
